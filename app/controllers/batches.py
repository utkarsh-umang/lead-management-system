...
async def upload_batch(
    ...
    # "This list was already sent/scheduled on Instantly" — the campaign
    # started outside LMS, so the contact event is backfilled at import
    # instead of being generated by an export. Without this, every
    # already-worked list looks untouched and the never-contacted count
    # (the number the dashboard leads with) is silently inflated.
    already_contacted_year: int | None = Form(default=None),
    already_contacted_month: int | None = Form(default=None),
) -> BatchSummary:
    if (already_contacted_year is None) != (already_contacted_month is None):
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
            "already_contacted_year and already_contacted_month must be given together",
        )
    if already_contacted_month is not None and not 1 <= already_contacted_month <= 12:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY, "already_contacted_month must be 1-12"
        )
    ...
    # Runs after ingestion has committed, so it's a second transaction:
    # this batch's leads don't exist to query until then. Only leads that
    # actually have an email — Instantly never received the others, so
    # claiming they were contacted would be a lie that also hides them from
    # the never-contacted count forever.
    if already_contacted_year is not None and already_contacted_month is not None:
        contacted_ids = (
            (
                await session.execute(
                    select(MasterLead.id)
                    .join(LeadSource, LeadSource.lead_id == MasterLead.id)
                    .where(LeadSource.batch_id == batch.id, MasterLead.email.is_not(None))
                    .distinct()
                )
            )
            .scalars()
        ...
        if contacted_ids:
            await record_contact_event(
                session,
                list(contacted_ids),
                date(already_contacted_year, already_contacted_month, 1),
            )
